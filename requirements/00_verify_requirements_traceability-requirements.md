...
Design: Infer test files from requirements path and scoped source conventions, including `Tests/sh`, top-level `Makefile` mapping to `Tests/sh/Makefile.bats`, and SwiftPM tests in `Tests/`, while canonicalizing symlinked test paths.
Tests:
